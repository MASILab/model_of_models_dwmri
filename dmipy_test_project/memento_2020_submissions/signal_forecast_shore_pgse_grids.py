...
def main():
    ...
    base_path = r'/nfs/masi/nathv/memento_2020/signal_forecast_data/files_project_2927_session_1436088/1-SignalForecast-ProvidedData/PGSE_grid'
    base_path = os.path.normpath(base_path)

    save_path = r'/nfs/masi/nathv/memento_2020/pgse_grids_submissions'
    save_path = os.path.normpath(save_path)
    if os.path.exists(save_path) == False:
        os.mkdir(save_path)

    # Read files via numpy load txt
    acq_params_file = np.loadtxt(os.path.join(base_path, 'PGSE_grids_provided_acq_params.txt'))
    pgse_provided_signals = np.loadtxt(os.path.join(base_path, 'PGSE_grids_provided_signals.txt'))
    pgse_unprovided_signals = np.loadtxt(os.path.join(base_path, 'PGSE_grids_unprovided_acq_params.txt'))
