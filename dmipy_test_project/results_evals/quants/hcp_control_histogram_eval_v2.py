...
def main():
    ...
    mask_dims = mask_bool.shape

    # Load the White Matter Mask
    wm_mask_path = r'/nfs/masi/nathv/miccai_2020/micro_methods_hcp_mini/115825/T1w_acpc_dc_restore_1.25_mixeltype.nii'
    wm_mask_path = os.path.normpath(wm_mask_path)
    wm_mask_data = load_nifty(wm_mask_path, data_type='float32')

    clean_wm_mask = np.zeros((mask_dims[0], mask_dims[1], mask_dims[2]))
    # Clean the WM mask as its quite dirty
    for x in range(mask_dims[0]):
        print(x)
        for y in range(mask_dims[1]):
            for z in range(mask_dims[2]):
                if mask_data[x,y,z] == 1:
                    if wm_mask_data[x,y,z] == 2:
                        clean_wm_mask[x, y, z] = 1
